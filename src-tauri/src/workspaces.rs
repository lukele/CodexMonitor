...
use crate::types::{
    Backend, WorkspaceEntry, WorkspaceInfo, WorkspaceKind, WorkspaceSettings, WorktreeInfo,
};
...
pub(crate) async fn add_workspace(
    ...
    {
        let workspaces = state.workspaces.lock().await;
        if let Some(existing) = workspaces.values().find(|w| w.path == path) {
            return Ok(WorkspaceInfo {
                id: existing.id.clone(),
                name: existing.name.clone(),
                path: existing.path.clone(),
                codex_bin: existing.codex_bin.clone(),
                connected: state.sessions.lock().await.contains_key(&existing.id),
    ...
    // Save workspace to state and disk immediately (before connecting)
    {
    ...
    // Return workspace info immediately with connected: false
    ...
    tokio::spawn(async move {
        eprintln!("[workspaces] Background connection task started for {}", entry_clone.id);
        
        // All workspaces use Pi backend now
        let session_result = spawn_workspace_session(
            entry_clone.clone(), 
            default_bin, 
            Backend::Pi, 
            app_clone.clone()
        ...
        match session_result {
            Ok(session) => {
                eprintln!("[workspaces] Connection succeeded for {}", entry_clone.id);
                // Store session in state
                app_clone.state::<crate::state::AppState>().sessions.lock().await.insert(entry_clone.id.clone(), session);
                // Emit connected event
                let emit_result = app_clone.emit("workspace-connected", serde_json::json!({
                    "workspaceId": entry_clone.id,
                    "connected": true
                }));
                eprintln!("[workspaces] Emitted workspace-connected: {:?}", emit_result);
            }
            Err(err) => {
                eprintln!("[workspaces] Connection failed for {}: {}", entry_clone.id, err);
                // Emit connection error event
                let _ = app_clone.emit("workspace-connection-error", serde_json::json!({
                    "workspaceId": entry_clone.id,
                    "error": err
                }));
            }
...
pub(crate) async fn add_worktree(
    ...
    let session = spawn_workspace_session(entry.clone(), default_bin, Backend::Pi, app).await?;
    {
        let mut workspaces = state.workspaces.lock().await;
        workspaces.insert(entry.id.clone(), entry.clone());
        let list: Vec<_> = workspaces.values().cloned().collect();
...
pub(crate) async fn connect_workspace(
    ...
    // All workspaces now use Pi backend (pi-adapter)
    let backend = Backend::Pi;
    let session = spawn_workspace_session(entry.clone(), default_bin, backend, app).await?;
