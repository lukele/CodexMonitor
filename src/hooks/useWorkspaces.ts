...
export function useWorkspaces(options: UseWorkspacesOptions = {}) {
  ...
  const [connectingWorkspaces, setConnectingWorkspaces] = useState<Set<string>>(new Set());
  ...
  // Listen for workspace connection events (async connection after add)
  useEffect(() => {
    console.log("[useWorkspaces] Setting up workspace-connected listener");
    const unlistenConnected = listen<{ workspaceId: string; connected: boolean }>(
      "workspace-connected",
      (event) => {
        console.log("[useWorkspaces] Received workspace-connected event:", event.payload);
        const { workspaceId, connected } = event.payload;
        setWorkspaces((prev) =>
          prev.map((ws) =>
            ws.id === workspaceId ? { ...ws, connected } : ws
          )
        );
        // Clear connecting state
        setConnectingWorkspaces((prev) => {
          const next = new Set(prev);
          next.delete(workspaceId);
          return next;
        });
        onDebug?.({
          id: `${Date.now()}-workspace-connected`,
          timestamp: Date.now(),
          source: "client",
    ...
    const unlistenError = listen<{ workspaceId: string; error: string }>(
      "workspace-connection-error",
      (event) => {
        const { workspaceId, error } = event.payload;
        console.log("[useWorkspaces] Received workspace-connection-error event:", event.payload);
        ...
        onDebug?.({
          id: `${Date.now()}-workspace-connection-error`,
          timestamp: Date.now(),
          source: "error",
  ...
  async function addWorkspace() {
    ...
    try {
      ...
      // Mark as connecting (background connection in progress)
      setConnectingWorkspaces((prev) => new Set(prev).add(workspace.id));
      return workspace;
    } catch (error) {
      onDebug?.({
