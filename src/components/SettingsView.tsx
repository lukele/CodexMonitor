...
import { getAuthStatus } from "../services/tauri";
...
type SettingsSection = "projects" | "display" | "providers" | "codex";
...
export function SettingsView({
  ...
  // Load auth providers when switching to providers section
  const loadAuthProviders = async () => {
    if (!activeWorkspaceId) return;
    setIsLoadingAuth(true);
    try {
      const response = await getAuthStatus(activeWorkspaceId);
  ...
  useEffect(() => {
    if (activeSection === "providers" && activeWorkspaceId) {
      loadAuthProviders();
    }
          ...
          <div className="settings-content">
            ...
            {activeSection === "providers" && (
              <section className="settings-section">
                ...
                <div className="settings-section-subtitle">
                  Manage authentication for different AI model providers.
                </div>
                <div className="settings-providers">
                  {isLoadingAuth ? (
                    ...
                    <div className="settings-empty">
                      Connect a workspace to view provider status.
                    </div>
                  ) : authProviders.length === 0 ? (
                    <div className="settings-empty">
                      No providers configured. Run `pi` in terminal to authenticate.
                    </div>
                  ) : (
                        ...
                        <div className="settings-provider-info">
                          <div className="settings-provider-name">
                            {provider.authenticated ? (
                              <CheckCircle size={16} className="provider-status-ok" />
                            ) : (
                              <XCircle size={16} className="provider-status-error" />
                            )}
                            {provider.name}
                          </div>
                          <div className="settings-provider-status">
                            {provider.authenticated ? (
                              provider.expired ? (
                                <span className="status-expired">Token expired</span>
                              ) : (
                                <span className="status-ok">Authenticated</span>
                              )
                            ) : (
                              <span className="status-missing">Not authenticated</span>
                            )}
                            {provider.expiresAt && !provider.expired && (
                ...
                <div className="settings-help" style={{ marginTop: "1rem" }}>
                  To authenticate with a new provider, run <code>pi --provider &lt;name&gt;</code> in Terminal.
                  <br />
                  Supported providers: anthropic, openai-codex, github-copilot, google-gemini-cli
                </div>
              </section>
            )}
            {activeSection === "codex" && (
              <section className="settings-section">
                ...
                <div className="settings-section-subtitle">
                  Configure the pi-adapter path and validate the installation.
                </div>
