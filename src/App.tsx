...
import "./styles/tool-preview.css";
import "./styles/code-block.css";
import { Sidebar } from "./components/Sidebar";
import { WorktreePrompt } from "./components/WorktreePrompt";
import { Home } from "./components/Home";
import { MainHeader } from "./components/MainHeader";
import { Messages, type ToolPreview } from "./components/Messages";
...
function MainApp() {
  ...
  const {
    ...
    onBackendSwitch: (backend) => {
      addDebugEntry({
        id: `${Date.now()}-backend-switched`,
        timestamp: Date.now(),
        source: "client",
        label: "Backend switched",
        payload: { backend },
      });
      // Refresh workspaces to get updated backend info
      refreshWorkspaces();
    },
  });
  ...
  function handleToolPreview(preview: ToolPreview | null) {
    setToolPreview(preview);
    // Tool preview shows in right panel, no need to change center mode
  }

  function handleCloseToolPreview() {
    setToolPreview(null);
  }
  ...
  const sidebarNode = (
      <Sidebar
        ...
        connectingWorkspaces={connectingWorkspaces}
